...
pub async fn request_work(
    ...
    if let Some(record) = connection.transaction::<Option<QueuedJob>, _, _>(|conn| {
        if let Some(record) = queue_base()
            .filter(queue::worker.is_null())
            .filter(build_inputs::architecture.eq_any(pop_request.supported_architectures))
            ...
            .map_err(Error::from)?
        {
            ...
            Ok::<Option<QueuedJob>, Error>(Some(record))
        } else {
            Ok(None)
        }
    })? {
